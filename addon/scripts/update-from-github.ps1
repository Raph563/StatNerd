param(
	[string]$Repository = 'Raph563/StatNerd',
	[string]$ReleaseTag = '',
...
try
{
	...
	$asset = $assets | Where-Object { $_.name -match '^statnerd-addon-v.+\.zip$' } | Select-Object -First 1
	if (-not $asset)
	{
	...
	if (-not (Compose-CustomJs -DataDir $dataDir -ComposeSourcesRaw $composeSourcesRaw -ActiveTargetFile $activeFile -ComposeEnabled $composeEnabled -SourceLabel 'StatNerd'))
	{
